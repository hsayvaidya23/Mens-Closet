...
const Suits = ({ products }) => {
  return (
    <div>
       <Head>
        <title>Suits -MensCloset.com</title>
        <meta
          name="viewport"
      ...
      <section className="text-gray-600 body-font min-h-screen">
        <div className="container px-5 py-24 mx-auto">
          <div className="flex flex-wrap -m-4 justify-center">
            {Object.keys(products).length === 0 && <p className='mx-5'>Sorry all the Suits are out of stock. New Stock coming soon! Stay Tuned!</p>}
...
export async function getServerSideProps(context) {
  ...
  for (let item of products) {
    if (item.title in suits) {
      if (!suits[item.title].color.includes(item.color) && item.availableQty > 0) {
        suits[item.title].color.push(item.color)
      }
      if (!suits[item.title].size.includes(item.size) && item.availableQty > 0) {
  ...
  return {
    props: { products: JSON.parse(JSON.stringify(suits)) }, // will be passed to the page component as props
  }
}
